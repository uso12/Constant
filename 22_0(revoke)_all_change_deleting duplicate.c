...
int main(void)
{
    ...
    length_ele_extended = sizeof(stack) / sizeof(stack[0]) * 4;//4种^
...
//--------------------------------single_thread_gen-----------------------------------------//
    ...
    double Exponent_delete,EPS_delete;
    ...
    for (i = 0; i < length_ele; i++)
    {
        ...
        stack_extended[i + 1 * length_ele] = powl(stack[i], 0.5);
        stack_extended[i + 2 * length_ele] = powl(stack[i], 1.5);
        stack_extended[i + 3 * length_ele] = powl(stack[i], 2);
    }
...
//--------------------------------------quick sort--------------------------------------------//
    ...
    count_del = 0;
    count = 0;
    //if (fabs(diva_minus - Target_diva_debug) < 4.5*pow(10,abs(Exponent_TDD)-6)) //exp<0
    //if (fabs(buffer_original[i] - buffer_original[i + 1]) > EPS_delete) 
    for (q_right = 1; q_right < total_pushed; q_right++) // 清除重复项
    {
        ...
        Exponent_delete = floor(log10(buffer_original[q_left]));//Exponent_TDD means number after E;
        if(buffer_original[q_left] < 0)
        {
            EPS_delete = 1*1/pow(10,abs(Exponent_delete)+6);
            if (fabs(buffer_original[q_left] - buffer_original[q_right]) > EPS_delete)//(buffer_original[q_left] != buffer_original[q_right])
            {
        ...
        {
            EPS_delete = 0.5*pow(10,abs(Exponent_delete)-6);
            if (fabs(buffer_original[q_left] - buffer_original[q_right]) < EPS_delete)//(buffer_original[q_left] != buffer_original[q_right])
            {
                buffer_original[++q_left] = buffer_original[q_right];
                count++;
...
#endif
#ifdef map
#ifdef print_find_EPS
        if (fabs(diva_minus - Target_diva_debug) < EPS)
        {
            local_count++;
            printf("1 %e\n", diva_minus);
        }
        else if (fabs(diva_plus - Target_diva_debug) < EPS)
        {
            local_count++;
            printf("1 %e\n", diva_plus);
        }
...
//打印结果
    ...
    buffer_shrink = (double *)malloc(list * sizeof(double));
    buffer_original = diva_t;
#ifdef print_find_EPS
    for (i = 0; i < local_count; i++) //
    {
        if (fabs(buffer_original[i] - Target_diva_debug) < EPS)
        {
            printf("2 %ld %e %ld %ld\n", i, buffer_original[i], i_t[i], j_t[i]);
        }
    }
#endif
