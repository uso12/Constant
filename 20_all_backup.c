...
#define Target_diva_debug 9.861434e-010//3.834622e-011//7.662571e-011//5.412208e-010 //old 9.861436e-010 9.861434e-010
...
//  幂函数分组归类
    ...
    j = 0;
    local_count = 0;
    for (i = All_pass_start; i < All_pass_start + count; i++) // i1_t[power_length-1] All_pass_end
    {
...
//打印结果
    ...
    for (i = 0; i < local_count; i++) // 清除重复项
    {
        ...
        if (fabs(buffer_original[i] - buffer_original[i + 1]) > EPS) // if (buffer_original[i] != buffer_original[i + 1])
        {                                                            // two method buffer[i] != buffer[i+1];buffer[i] <= buffer[i+1] * (double)0.9999999999;
