...
// #define print_multi_thread_first_past
...
int *i_t, *j_t;
    ...
    unsigned long long total_pushed=0, zero = 0;                                     //所有线程的生成个数之和
    double *buffer_shrink,*buffer_shrink_save;              //删除后留下的数组
    struct Param {
	char a;
	char b;
...
int qsort_cmp(const void *a, const void *b);
    ...
    void* quicksort_thread(void* args)
    {
        ...
        qsort(buffer_o_thread[tid], total_pushed_thread[tid], sizeof(double), qsort_cmp);
...
//--------------------------------single_thread_gen-----------------------------------------//
    ...
    //调整buffer_extended后要修改
    #ifdef multi_thread
        buffer_o_thread[0] = buffer_original+0;
        buffer_o_thread[1] = buffer_original+779700511LL;
        buffer_o_thread[2] = buffer_original+1565900826LL;
        buffer_o_thread[3] = buffer_original+2345087012LL;
        buffer_o_thread[4] = buffer_original+3137349048LL;
        buffer_o_thread[5] = buffer_original+3918557651LL;
        buffer_o_thread[6] = buffer_original+4695001136LL;
        buffer_o_thread[7] = buffer_original+5476372154LL;
        total_pushed_thread[0] = 0;
        total_pushed_thread[1] = 0;
        total_pushed_thread[2] = 0;
        total_pushed_thread[3] = 0;
        total_pushed_thread[4] = 0;
        total_pushed_thread[5] = 0;
        total_pushed_thread[6] = 0;
        total_pushed_thread[7] = 0;
    #endif
...
//108s before sorting the data //210s add Exponent_TDD
    #ifdef multi_thread
    int expo[470],max=0,min=0;
                            ...
                            if(m % length_ele != o % length_ele)
                            {
                                *buffer_original = stack_extended[i] * stack_extended[j] * stack_extended[k] * stack_extended[l] * stack_extended[m] * stack_extended[o];
                                ...
                                else
                                {
                                        ...
                                        exponent = 324+(int)Exponent_TDD;
                                        if(exponent < 120)
                                        {
                                            *buffer_o_thread[0] = *buffer_original;
                                            buffer_o_thread[0]++;
                                            total_pushed_thread[0]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 165)
                                        {
                                            *buffer_o_thread[1] = *buffer_original;
                                            buffer_o_thread[1]++;
                                            total_pushed_thread[1]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 198)
                                        {
                                            *buffer_o_thread[2] = *buffer_original;
                                            buffer_o_thread[2]++;
                                            total_pushed_thread[2]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 227)
                                        {
                                            *buffer_o_thread[3] = *buffer_original;
                                            buffer_o_thread[3]++;
                                            total_pushed_thread[3]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 253)
                                        {
                                            *buffer_o_thread[4] = *buffer_original;
                                            buffer_o_thread[4]++;
                                            total_pushed_thread[4]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 277)
                                        {
                                            *buffer_o_thread[5] = *buffer_original;
                                            buffer_o_thread[5]++;
                                            total_pushed_thread[5]++;
                                            total_pushed++;
                                        }
                                        else if(exponent < 303)
                                        {
                                            *buffer_o_thread[6] = *buffer_original;
                                            buffer_o_thread[6]++;
                                            total_pushed_thread[6]++;
                                            total_pushed++;
                                        }
                                        else
                                        {
                                            *buffer_o_thread[7] = *buffer_original;
                                            buffer_o_thread[7]++;
                                            total_pushed_thread[7]++;
                                            total_pushed++;
                                        }
                                        #ifdef print_multi_thread_first_past
                                            if((int)Exponent_TDD > max) max = (int)Exponent_TDD;
                                            if((int)Exponent_TDD < min) min = (int)Exponent_TDD;
                                            if((int)Exponent_TDD < -500) printf("buffer_original = %e Exponent_TDD = %d\n", *buffer_original),(int)Exponent_TDD;//buffer-equal-zero
                                            //max = 142 min = -324
                                            expo[324+(int)Exponent_TDD]++;
                                        #endif
                                    if (total_pushed % 50000000 == 0)
                                        printf("push = %lld buffer_current = %e Exponent_TDD = %d\n", total_pushed, *buffer_original,(int)Exponent_TDD);
                                    #endif
                                    #ifndef multi_thread
                                    buffer_original++;
                                    total_pushed++;
...
#ifdef multi_thread
    *buffer_original = stack_extended[0] * stack_extended[1] * stack_extended[2] * stack_extended[3] * stack_extended[4] * stack_extended[5];
#endif
#ifdef print_multi_thread_first_past
    printf("max = %d min = %d\n",max,min);
    for (i = 0; i < 467; i++)
        printf("i = %d number = %d\n", i, expo[i]);
#endif
    printf("total_pushed_thread[0] = %lld total_pushed_thread[1] = %lld total_pushed_thread[2] = %lld total_pushed_thread[3] = %lld\n",
    total_pushed_thread[0],total_pushed_thread[1],total_pushed_thread[2],total_pushed_thread[3]);
    printf("count = %ld count_del = %ld total_pushed = %lld\n", count, count_del, total_pushed);
...
//--------------------------------------quick sort--------------------------------------------//
    ...
    printf("before sort\n");
    printf("total_pushed = %lld\n", total_pushed);
    printf("original buffer before quicksort tail=%e tail+1 = %e head = %e head+1 = %e\n", 
        buffer_original[total_pushed - 1], buffer_original[total_pushed], buffer_original[0], buffer_original[1]);
    printf("head10\n");
    for (i = 0; i < 10; i++)
        printf("count = %ld buffer_original[i] = %e\n", i, buffer_original[i]);
    printf("tail10\n");
    for (i = total_pushed - 10; i < total_pushed; i++)
        printf("count = %lld buffer_original[i] = %e\n", i, buffer_original[i]);
    printf("all sample\n");
    for (i = 0; i < total_pushed; i+= 50000000)
        printf("count = %lld buffer_original[i] = %e\n", i, buffer_original[i]);
#endif
#ifdef multi_thread
        ...
        for (i = 0; i < total_pushed; i+= 5000000)
            printf("count = %lld buffer_original[i] = %e\n", i, buffer_original[i]);
        // if use original qsort at the end the result will be slower
